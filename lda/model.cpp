...
int model::save_model(string model_name) {
    ...
    /* new dt output what i added */
    if (save_model_dt(dir + model_name + dt_suffix)) {
      return 1;
    }
...
/* new dt output */
int model::save_model_dt(string filename) {
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
    	printf("Cannot open file %s to save!\n", filename.c_str());
    	return 1;
...
int model::save_inf_model(string model_name) {
    ...
    /* inference phase dt output */
    if (save_inf_model_newdt(dir + model_name + dt_suffix)) {
      return 1;
    }
...
int model::save_inf_model_newdt(string filename) {
    int i, j;
